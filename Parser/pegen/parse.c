...
#define aleq_bitwise_or_type 1084
...
static CmpopExprPair *aleq_bitwise_or_rule(Parser *p);
...
type_expressions_rule(Parser *p)
{
    ...
    { // ','.expression+ ',' '*' expression ',' '**' expression
        ...
        if (
            ...
            &&
            (_literal_3 = _PyPegen_expect_token(p, 36)) // token='**'
            &&
    ...
    { // ','.expression+ ',' '**' expression
        ...
        if (
            ...
            (_literal = _PyPegen_expect_token(p, 12)) // token=','
            &&
            (_literal_1 = _PyPegen_expect_token(p, 36)) // token='**'
            &&
            (b = expression_rule(p)) // expression
        )
        {
    ...
    { // '*' expression ',' '**' expression
        ...
        if (
            ...
            &&
            (_literal_2 = _PyPegen_expect_token(p, 36)) // token='**'
            &&
            (b = expression_rule(p)) // expression
        )
        {
...
compare_op_bitwise_or_pair_rule(Parser *p)
{
    ...
    { // aleq_bitwise_or
        ...
        D(fprintf(stderr, "%*c> compare_op_bitwise_or_pair[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "aleq_bitwise_or"));
        CmpopExprPair *aleq_bitwise_or_var;
        if (
            (aleq_bitwise_or_var = aleq_bitwise_or_rule(p)) // aleq_bitwise_or
        )
        {
            D(fprintf(stderr, "%*c+ compare_op_bitwise_or_pair[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "aleq_bitwise_or"));
            _res = aleq_bitwise_or_var;
            goto done;
        }
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s compare_op_bitwise_or_pair[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "aleq_bitwise_or"));
    }
    { // noteq_bitwise_or
        if (p->error_indicator)
        {
...
// aleq_bitwise_or: '~=' bitwise_or
static CmpopExprPair *
aleq_bitwise_or_rule(Parser *p)
{
    D(p->level++);
    if (p->error_indicator)
    {
    ...
    { // '~=' bitwise_or
        ...
        D(fprintf(stderr, "%*c> aleq_bitwise_or[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "'~=' bitwise_or"));
        Token *_literal;
        expr_ty a;
        if (
        ...
        {
            D(fprintf(stderr, "%*c+ aleq_bitwise_or[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'~=' bitwise_or"));
            _res = _PyPegen_cmpop_expr_pair(p, AlEq, a);
            if (_res == NULL && PyErr_Occurred())
            {
                p->error_indicator = 1;
                D(p->level--);
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s aleq_bitwise_or[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "'~=' bitwise_or"));
    }
    _res = NULL;
done:
...
invalid_dict_comprehension_rule(Parser *p)
{
    ...
    { // '{' '**' bitwise_or for_if_clauses '}'
        ...
        if (
            (_literal = _PyPegen_expect_token(p, 25)) // token='{'
            &&
            (a = _PyPegen_expect_token(p, 36)) // token='**'
            &&
            (bitwise_or_var = bitwise_or_rule(p)) // bitwise_or
            &&
            (for_if_clauses_var = for_if_clauses_rule(p)) // for_if_clauses
...
_loop0_1_rule(Parser *p)
{
    ...
    for (int i = 0; i < _n; i++)
        asdl_seq_SET(_seq, i, _children[i]);
    PyMem_Free(_children);
...
_tmp_91_rule(Parser *p)
{
    ...
    { // '!='
        ...
        Token *tok;
        if (
            (tok = _PyPegen_expect_token(p, 29)) // token='!='
        )
        {
