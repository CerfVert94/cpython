...
void _PyAST_Fini()
{
    ...
    Py_CLEAR(state->AlEq_singleton);
    Py_CLEAR(state->AlEq_type);
...
static int init_types(astmodulestate *state)
{
    ...
    state->cmpop_type = make_type(state, "cmpop", state->AST_type, NULL, 0,
        "cmpop = Eq | NotEq | Lt | LtE | Gt | GtE | Is | IsNot | In | NotIn | AlEq");
    ...
    state->AlEq_type = make_type(state, "AlEq", state->cmpop_type, NULL, 0,
        "AlEq");
    if (!state->AlEq_type) return 0;
    state->AlEq_singleton = PyType_GenericNew((PyTypeObject *)state->AlEq_type,
                                              NULL, NULL);
    if (!state->AlEq_singleton) return 0;
...
PyObject* ast2obj_cmpop(astmodulestate *state, cmpop_ty o)
{
    switch(o) {
        ...
        case AlEq:
            Py_INCREF(state->AlEq_singleton);
            return state->AlEq_singleton;
    }
    Py_UNREACHABLE();
}
PyObject*
              ...
              arena)
{
    ...
    isinstance = PyObject_IsInstance(obj, state->AlEq_type);
    if (isinstance == -1) {
        return 1;
    }
    if (isinstance) {
...
astmodule_exec(PyObject *m)
{
    ...
    if (PyModule_AddObject(m, "AlEq", state->AlEq_type) < 0) {
        return -1;
    }
    Py_INCREF(state->AlEq_type);
